defmodule KeyboardLayoutTest do
  ...
  #   %{id: :l3, x: 3, y: 3}
  # ]
  test "KeyboardLayout.load_from_config/0 loads the layout defined in the application config" do
    assert KeyboardLayout.load_from_config() == %KeyboardLayout{
             keys: [
  ...
  defp add_layout(_context) do
    [layout: KeyboardLayout.load_from_config()]
  end
  ...
  test "new/1 takes only a list of keys and returns a %KeyboardLayout{} struct" do
    keys = [Key.new(:k, 0, 0)]
  ...
  test "new/2 takes a list of keys and a list of LEDs and returns a %KeyboardLayout{} struct" do
    keys = [Key.new(:k1, 0, 0), Key.new(:k2, 1, 0, led: :l2)]
    leds = [LED.new(:l1, 0, 0), LED.new(:l2, 1, 0)]

    assert KeyboardLayout.new(keys, leds) == %KeyboardLayout{
             keys: [
               %KeyboardLayout.Key{height: 1, id: :k1, led: nil, width: 1, x: 0, y: 0},
               %KeyboardLayout.Key{height: 1, id: :k2, led: :l2, width: 1, x: 1, y: 0}
             ],
             keys_by_leds: %{
               l2: %KeyboardLayout.Key{height: 1, id: :k2, led: :l2, width: 1, x: 1, y: 0}
             },
             leds: [
               %KeyboardLayout.LED{id: :l1, x: 0, y: 0},
               %KeyboardLayout.LED{id: :l2, x: 1, y: 0}
             ],
             leds_by_keys: %{
               k2: %KeyboardLayout.LED{id: :l2, x: 1, y: 0}
             }
           }
  end
  ...
  test "keys/1 takes a layout and returns its list of keys",
    ...
    assert KeyboardLayout.keys(layout) == keys
    assert KeyboardLayout.keys(layout) == layout.keys
  end

  test "leds/1 takes a layout and returns its list of LEDs",
       %{layout: layout, leds: leds} do
    assert KeyboardLayout.leds(layout) == leds
    assert KeyboardLayout.leds(layout) == layout.leds
  end
  ...
  test "led_for_key/2 takes a layout and a key id and returns the corresponding LED or nil",
    ...
    led_for_k3 = nil
    assert KeyboardLayout.led_for_key(layout, :k1) == led_for_k1
    assert KeyboardLayout.led_for_key(layout, :k3) == led_for_k3
  end
  ...
  test "key_for_led/2 takes a layout and a LED id and returns the corresponding key or nil",
    ...
    key_for_l3 = nil
    assert KeyboardLayout.key_for_led(layout, :l1) == key_for_l1
    assert KeyboardLayout.key_for_led(layout, :l3) == key_for_l3
  end
end
